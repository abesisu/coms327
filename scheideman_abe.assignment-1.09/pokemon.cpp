                          ...
                          int special_defense_iv)
{
    ...
    for (i = 0; i < data->pokemon_stats.size(); i++) {
        if (data->pokemon_stats[i].pokemon_id == data->pokemon_data[index].id) {  // check that these are right
            if (data->pokemon_stats[i].stat_id == data->stats[0].id) {
                ...
                attack = (((data->pokemon_stats[i].base_stat + hp_iv) * 2 * level) / 100) + level + 10;
            } else if (data->pokemon_stats[i].stat_id == data->stats[2].id) {
                defense = (((data->pokemon_stats[i].base_stat + defense_iv) * 2 * level) / 100) + 5;
            } else if (data->pokemon_stats[i].stat_id == data->stats[3].id) {
                special_attack = (((data->pokemon_stats[i].base_stat + special_attack_iv) * 2 * level) / 100) + 5;
            } else if (data->pokemon_stats[i].stat_id == data->stats[4].id) {
                special_defense = (((data->pokemon_stats[i].base_stat + special_defense_iv) * 2 * level) / 100) + 5;
            } else if (data->pokemon_stats[i].stat_id == data->stats[5].id) {
                speed = (((data->pokemon_stats[i].base_stat + speed_iv) * 2 * level) / 100) + 5;
                base_speed = data->pokemon_stats[i].base_stat;
            }
        }
    ...
    for (i = 1; i <= data->pokemon_types.size(); i++) {
        if (data->pokemon_types[i].pokemon_id == data->pokemon_data[index].id) {
            if (types[0] == 0) {
                types[0] = data->pokemon_types[i].type_id;
            } else {
                types[1] = data->pokemon_types[i].type_id;
            }
        }
        if (data->pokemon_types[i].pokemon_id > data->pokemon_data[index].id) {
            break;
